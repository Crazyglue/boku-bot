...
import { AppMentionEvent, SayArguments } from '@slack/bolt';
...
const generateCurseResponse = async (event: AppMentionEvent): Promise<ChatPostMessageArguments> => ({ text: `<@${event.user}> thats very rude, why would you say that?`, channel: event.channel });
const generateDefaultResponse = async (event: AppMentionEvent): Promise<ChatPostMessageArguments> => ({ text: `<@${event.user}> I tried to think of a response, but I'm too dumb.`, channel: event.channel });

type CheckFunction = (event: string) => boolean;
type EventHandler = (event: AppMentionEvent) => Promise<ChatPostMessageArguments>;
...
export default async function handleEvent(event: AppMentionEvent): Promise<SayArguments> {
