...
export async function generateAiMeme({ text: fullText, channel }: AppMentionEvent): Promise<ChatPostMessageArguments> {
    const log = logger.child({ functionName: 'generateAiMeme' });
    const requestText = fullText.replace(/<@.+>/g, '').replace('!generate', '').trim();  // remove boku user
    ...
    const sentences = aiText
        ...
        .map((sentence: string) => sentence.trim()).filter(Boolean); // trim and remove empty strings
    ...
    const otherTextCandidates = OTHER_TEXT_FILTERS.reduce((remaining, filterFn) => remaining.filter((r: string) => filterFn(r, topTextCandidates)), sentences);
